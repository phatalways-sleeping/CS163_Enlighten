...
bool login(string username, string password, string path, Node*& search_history, Node*& favorite_list) {
    ...
    vector<vector<string>> v;  vector<string> k;
    string line, word;
    if (file.is_open()) {
    ...
    for (int i = 0; i < v.size(); i++) {
        if (v[i][0] == username && v[i][1] == password) {
            int j = 3;
            while (v[i][j] != "favorite_list") insert(search_history, v[i][j++]);
            j += 1;
            while (j < v[i].size()) insert(favorite_list, v[i][j++]);
            return true;
        }
    }
    return false;
...
bool createNewAccount(string username, string password, string path) {
    ...
    file.close();
    k.clear(); k.push_back(username); k.push_back(password); k.push_back("search_history"); k.push_back("favorite_list");
    v.push_back(k);
    ofstream f(path, ios::out);
    if (f.is_open()) {
        for (int i = 0; i < v.size(); i++) {
            ...
            for (int i = 0; i < v.size(); i++) {
                if (v[i][0] == username) {
                    ...
                    for (int j = 3; j < v[i].size(); j++) {
                        if (v[i][j] == "favorite_list") check = true;
                        if (check) f << v[i][j] << ',';
                    }
                }
                else {
